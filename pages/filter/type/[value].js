...
export async function getStaticProps({ params: { value } }) {
  ...
  const dataList = (await Promise.all(collectionList.map(async collection => ff.readJson(ff.path(`./data/${collection}_data.json`))))).flat(); const filtered = dataList.filter(d => d.filters?.[FILTER_NAME]?.toLowerCase() == value?.toLowerCase()).sort(byWeight);
  const retval = [];
